...
res=np.genfromtxt("biglistofres.aei",unpack=True,usecols=(6),dtype=None)
f=Mtof(e,M/180.*np.pi)

dx=[]
dy=[]
for i in np.arange(0,len(a)):
  dx1,dy1,dz1=el2rv(1.0,a[i],e[i],inc[i]/180.*np.pi,Om[i]/180.*np.pi,om[i]/180.*np.pi,f[i])
  #print (x1**2.+y1**2.)**0.5
  dx=np.append(dx,dx1)
  dy=np.append(dy,dy1)
...
#plt.savefig('toyres.jpg')
plt.savefig('plutinos.jpg')
plt.show()
